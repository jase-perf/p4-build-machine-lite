...
def get_the_code(change, shelved):
...
# ---------------------------------------------------------------------------
def request_build(reason, shelf=None, review_url=None, force=False):
    """Ask for a build of the newest submitted change, or of one shelved change.
    ...
    with new_request:
        ...
        for waiting in queue:
            if waiting["shelf"] == shelf:          # the same build is already waiting: join it
                job = waiting
        if job is None:
            if len(queue) >= MAX_WAITING:
                return False
            job = {"shelf": shelf, "reason": reason, "review_urls": [], "force": False}
            queue.append(job)
...
def run_build(job):
    ...
    if not shelved and change == last_built_change() and not job["force"] and not job["review_urls"]:
    ...
    build = {"number": number, "change": change, "shelved": shelved, "user": user, "desc": desc,
    ...
    with open(folder / "log.txt", "wb") as log:
        ...
        passed = False
        try:
            say(f"Build {number}: {'shelved' if shelved else 'submitted'} change {change} by {user}")
            get_the_code(change, shelved)
            say(f"Running {BUILD_SCRIPT}")
            code = run_script(output, change, number, log)
            say(f"{BUILD_SCRIPT} exited with code {code}")
            passed = code == 0
            if passed and any(output.iterdir()):
                zip_name = f"{NAME}-{'shelf' if shelved else 'cl'}{change}"
...
def run_script(output, change, number, log):
...
def delete_old_builds():
    keep = newest_build(good=True)     # the newest good build stays downloadable, however old
...
def notify(build, review_urls):
    ...
    if WEBHOOK_URL and build["result"] != "running":
        ...
        kind = "Shelf" if build["shelved"] else "Change"
        post_json(WEBHOOK_URL, {
            "content": f"{icon} {NAME} build {build['number']} {build['result']}: {kind} "
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            good = newest_build(good=True)
            if good:
                self.redirect(zip_link(good))
            else:
                self.send(404, "No passing build yet\n")
    ...
    def do_POST(self):
        ...
        if params.get("status") == "shelved":       # Code Review's {status} for a review's shelf
            ...
            queued = request_build(reason, shelf=change, review_url=review_url)
        else:                                        # anything else: build the newest change
            queued = request_build(reason, review_url=review_url, force=params.get("force") == "1")
...
<input type="hidden" name="force" value="1">{token_box}<button>Build now</button></form>
{banner}
<p>{"Waiting: " + ", ".join(waiting) if waiting else ""}</p>
<table><tr><th>#</th><th>Result</th><th>Change</th><th>Who</th><th>What</th><th>Why</th>
